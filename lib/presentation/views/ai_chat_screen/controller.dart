...
class _ViewState {
  _ViewState({required this.history, required this.model, required this.chat, required this.loading});
  ...
  _ViewState.initial() : this(history: [], model: null, chat: null, loading: LoadingState.init);

  _ViewState copyWith({List<Content>? history, GenerativeModel? model, ChatSession? chat, LoadingState? loading}) {
    return _ViewState(
...
class _VSController extends StateNotifier<_ViewState> {
  ...
  void initState() {
    final model = GenerativeModel(model: 'gemini-pro', apiKey: MyAppX.environment.googleGenerativeAIKey);
    final chat = model.startChat();
    state = state.copyWith(model: model, chat: chat, loading: LoadingState.init, history: params.initialHistory);
  }
  ...
  void sendChatMessage(String message, BuildContext context) {
    ...
    try {
      var responseStream = state.chat?.sendMessageStream(
        Content('user', [TextPart(params.inputPrompt), TextPart(message)]),
      );
      var currentHistory = state.history;
      currentHistory.add(Content('user', [TextPart(message)]));
        ...
        (chunk) {
          ...
          if (text == null) {
            ...
            if (lastModelMessage != null) {
              ...
              final newModelMessage = Content('model', [
              ...
              var currentHistory = state.history;
              currentHistory.add(Content('model', [TextPart(text)]));
              state = state.copyWith(history: currentHistory);
            }
            scrollDown();
          }
